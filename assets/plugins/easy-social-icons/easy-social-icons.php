...
$pluginsURI = get_template_directory_uri() . '/assets/plugins/easy-social-icons/';
...
function cnss_add_menu_pages() {
	add_theme_page('Manage Social Icon', 'Easy Social Icon', 'manage_options', 'cnss_social_icon_page', 'cnss_social_icon_page_fn', get_template_directory_uri() . '/assets/plugins/easy-social-icons/images/scc-sc.png' );
	add_theme_page('Add Social Icons', 'Add Icons', 'manage_options', 'cnss_social_icon_add', 'cnss_social_icon_add_fn', get_template_directory_uri() . '/assets/plugins/easy-social-icons/images/scc-sc.png' );
...
function cnss_social_icon_option_fn() {
	...
	<form method="post" action="options.php">
		...
		<p class="submit">
		<input type="submit" class="button-primary" value="<?php _e('Save Changes', 'Evyr2014') ?>" />
		</p>
	</form>
	</div>
	<?php 
...
function cnss_db_install () {
   ...
   global $cnss_db_version;
	...
	$srcdir   = ABSPATH.'wp-content/themes/Evyr/assets/plugins/easy-social-icons/images/icon/';
...
if (isset($_POST['submit_button'])) {
	...
	if ($_POST['action'] == 'update')
	{
		...
		if ($err == '')
		{
			...
			"VALUES ('" . 
			esc_sql( $_REQUEST['title']) . "','" . 
			esc_sql( $_REQUEST['url']) . "','" . 
			$image_file_name . "'," . 
			$_REQUEST['sortorder'] . ",'" . 
			time() . "'," . 
			$_REQUEST['target'] . "" . 
			...
			<?php foreach($video_info as $vdoinfo) { ?>
					...
					<table width="100%">
					  <tr style="background:#f7f7f7">
						<td width="60">&nbsp;<img src="<?php echo $image_file_path.$vdoinfo->image_url;?>" width="<?php echo $cnss_width ?>" height="<?php echo $cnss_height ?>" alt="<?php echo $vdoinfo->title;?>" /></td>
...
<form method="post" enctype="multipart/form-data" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
        ...
        <tr valign="top">
			...
			<td>
				<?php if (isset($_GET['mode'])) { ?>
					<br /><img src="<?php echo $image_url?>" width="<?php echo $cnss_width ?>"  height="<?php echo $cnss_height ?>" alt="<?php echo $title?>" /><br />
				<?php } ?>
    ...
    <p class="submit">
    <input type="submit" id="submit_button" name="submit_button" class="button-primary" value="<?php _e('Save Changes', 'Evyr2014') ?>" />
    </p>
...
function cn_social_icon() {
	...
	ob_start();
	echo '<table class="cnss-social-icon" style="width:'.$table_width.'px" >';
	...
	?><td style="width:<?php echo $td_width ?>px"><a <?php echo ($icon->target==1)?'target="_blank"':'' ?> title="<?php echo $icon->title ?>" href="<?php echo $icon->url ?>"><img src="<?php echo $image_url?>" width="<?php echo $cnss_width ?>" height="<?php echo $cnss_height ?>" alt="<?php echo $icon->title ?>" /></a></td><?php 
...
class Cnss_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			array( 'description' => __( 'Easy Social Icon Widget for sidebar', 'Evyr2014' ) ) // Args
		);
	}
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'New title', 'Evyr2014' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'Evyr2014' ); ?></label> 
